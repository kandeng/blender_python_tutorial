# rabbitmq_service.py (full corrected code)
import aio_pika
import asyncio
import json
import os
    ...
    async def close(self):
        ...
        if self._connection and not self._connection.is_closed:
            try:
                ...
                warn_msg = f"close(), Following exception was thrown when closing RabbitMQ connection: '{str(e)}'."
                self.logger.warning(warn_msg)
...
class RabbitMQService:
    def __init__(
        ...
        try:
            self.logger = Logger("rabbit_mq").getLogger()
            debug_msg = f"RabbitMQService(), RabbitMQService '{self.service_name}' initialized successfully."
            self.logger.debug(debug_msg)

        except Exception as e:
            warn_msg = f"RabbitMQService(), Failed to initialize RabbitMQService '{self.service_name}', "
    ...
    async def start_consuming(
        ...
        try:
            ...
            debug_msg = f"start_consuming(), Microservice '{self.service_name}' started, \n"
            ...
            warn_msg = f"start_consuming(), Fail to start the microservice '{self.service_name}' "
    ...
    async def close(self):
        ...
        debug_msg = f"close(), Microservice '{self.service_name}' is successfully closed."
        self.logger.debug(debug_msg)