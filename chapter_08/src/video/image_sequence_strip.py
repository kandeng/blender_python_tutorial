...
class ImageSequenceStrip:
    ...
    def upload_image_sequence(
        ...
        # 2. Initialize video sequencer.
        if not bpy.context.scene.sequence_editor:
            bpy.context.scene.sequence_editor_create()
        sequencer = bpy.context.scene.sequence_editor

        # 3. Add image sequence strip to video sequence editor.
        ...
        try:
            ...
            self.strip_content = sequencer.active_strip
            if not self.strip_content and sequencer.strips:
                self.strip_content = sequencer.strips[-1]
            ...
            if self.strip_content:
                ...
                raise RuntimeError("Failed to add image sequence strip.")
        except Exception as e:
            self.logger.error(f"upload_image_sequence(), exception: {str(e)}")
            return
    ...
    def assemble_image_sequence_to_mp4(
        ...
        imgseq_scene.sequence_editor_create()
        sequencer = imgseq_scene.sequence_editor
        
        try:
            # 3. Upload the image sequence to Blender video sequence editor.
            ...
            # If no sequencer area exists, temporarily change one
            ...
            # Create image sequence strip using the operator with proper context
            files = [{"name": os.path.basename(f)} for f in self.frame_image_filenames]
            
            # Use the operator with the proper area context
            if sequencer_area:
                with bpy.context.temp_override(area=sequencer_area, scene=imgseq_scene):
                    bpy.ops.sequencer.image_strip_add(
                        directory=image_sequence_dir,
                        files=files,
                        relative_path=False,
            ...
            # Restore area type if we changed it temporarily
            if temp_area_change:
    ...
    def disassemble_video_to_image_sequence(
        ...
        try:
            ...
            temp_scene.frame_end = strip.frame_final_end  # Total frames
    ...
    def run_demo():
        ...
        output_video = "/home/robot/movie_blender_studio/output/bicycling_greenscreen.MP4"
