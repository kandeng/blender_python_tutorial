...
class EditorNode:
    ...
    def _create_node_tree(self):
        if len(self.editor_type) == 0:
            ...
            self.material = bpy.data.materials.new(name=self.editor_name)
    ...
    def reset(
        ...
        try:
            ...
            self._create_node_tree()  
            self.logger.info(f"Reset compositor nodes.")

        except Exception as e:
            warn_msg = f"reset(), Could not reset Compositor class, "
