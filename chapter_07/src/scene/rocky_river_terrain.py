...
class RockyRiverTerrain:
    def __init__(self, plane_width=10, plane_length=20):
        ...
        try:
            ...
            self.hdri_background = HdriBackground()

            from camera.camera import Camera
            self.camera = Camera("RiverbedInWild")
    ...
    def run_demo():
        ...
        # ---------------------------------------
        # 1. Create the 3D riverbed terrain 
        # ---------------------------------------        
        print("[INFO] --- 1. Create the 3D riverbed terrain ---")
        ...
        hdri_filename = "/home/robot/movie_blender_studio/asset/hdri/kloppenheim_06_4k.exr" 
        rocky_river_terrain.hdri_background.setup_hdri_background(
            hdri_filename=hdri_filename,
            light_strength=1.5
        )
        ...
        # ---------------------------------------
        # 2. Create and move camera 
        # ---------------------------------------
        print("[INFO] --- 2. Create and move camera ---")

        rocky_river_terrain.camera.reset_animation()
        rocky_river_terrain.camera.set_activate()
        rocky_river_terrain.camera.camera.data.lens = 20.0
        camera_properties = rocky_river_terrain.camera.get_properties()
        print(f"\n[INFO] Camera properties: ")
        pprint.pprint(camera_properties)

        camera_track_name = "CameraTrackCurve"
        camera_track_points = [(13, -22, 15.4), (-10, -16, -3)]
        camera_track_points += [(-0.5, -5, 0), (1, 0.5, 0), (1, 2.5, -1), (3, 7, -1)]
        camera_track_points += [(2, 13, -1), (-2.6, 20, 1), (0, 23, 19), (0, 0, 25)]
        keyframe_range = (1, 500)

        rocky_river_terrain.camera.animation.move_on_track(
            curve_name=camera_track_name,
            curve_coordinates=camera_track_points,
            keyframe_range=keyframe_range           
        )

        rocky_river_terrain.camera.animation.track_ahead(
            curve_name=camera_track_name, 
            keyframe_range=keyframe_range
        )        
        ...
        # --------------------------
        # 4. Rendering a MP4 video.
        # --------------------------
        rocky_river_terrain.camera.set_activate()
        rocky_river_terrain.camera.renderer.render_frame_images(
            output_path="output/tmp_images"
        )
        rocky_river_terrain.camera.renderer.compile_images_to_video(
