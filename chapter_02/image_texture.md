...
&nbsp;
## 4. A more complex task

[How to Setup and Use Texture Maps for Beginners](https://www.youtube.com/watch?v=fUZHyoeuwVI) 
is a Blender video tutorial on youtube by Ryan King. 

In this video tutorial, Ryan showed how to generate a wood floor that looks like real, using Blender 3D tool manually. 
...
1. Generate the Blender python script from scratch,

   ~~~
   Can you follow the video @https://www.youtube.com/watch?v=fUZHyoeuwVI to generate a Blender python script, 
   that creates a floor with color map, displacement map, roughness map, normal GL map.
   The texture images can be found in the file directory @/home/robot/blender_assets/texture/WoodFloor043_4K. 
   ~~~

   Google gemini-cli responds that,
   * It can't watch external video.
   * Also, it can't read any files that resides outside the current file directory,
     so that it can't read the texture mapping images. 

   While Qwen3-coder,
   * It can read the texture mapping images,
     even though they are stored in a file directory that is outside the current directory.
   * But it doesn't say whether or not it can watch the external video.

   Therefore, we modify the prompt, and use Qwen3-coder to generate the Blender python script,

   ~~~
   Can you generate a Blender python script, 
   that creates a floor with color map, displacement map, roughness map, normal GL map. 
   The texture images can be found in the file directory @/home/robot/blender_assets/texture/WoodFloor043_4K. 
   ~~~

2. Followings are the sample prompts to ask Qwen3-coder to debug and modify the Blender python script.

   Fix bug,
   ~~~
   In @/home/robot/movie_blender_studio/wood_floor_texture.py, 
   it needs to add a link from "principled_node"'s "BSDF" output, to "output_node"'s "Surface" input.    
   ~~~
   ~~~
   When running @/home/robot/movie_blender_studio/wood_floor_texture.py, 
   it threw an exception in line 227 "scene.eevee.use_ssr = True", 
   complaining that "AttributeError: 'SceneEEVEE' object has no attribute 'use_ssr'". 
   Can you please fix the bug?
   ~~~

   Modify the script,
   ~~~
   Can you please modify the Blender python script @/home/robot/movie_blender_studio/wood_floor_texture.py,
   to delete the cube object sitting on the floor?    
   ~~~
   
3. Qwen3-coder generated a Blender python script, and named it as ["wood_floor_texture.py"](./src/wood_floor_texture.py).

   After loading, running and debugging this Blender python script, the final result is displayed in the video below.

   [![Python controls the Blender's camera movement](https://img.youtube.com/vi/_dpmmBj0LC8/hqdefault.jpg)](https://www.youtube.com/watch?v=_dpmmBj0LC8)
