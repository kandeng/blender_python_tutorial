...
class MovieEditor:
    def __init__(self):
        ...
        try:
            ...
            self.logger = LlamediaLogger("MovieEditor").getLogger()

            self.logger.info(f"MovieEditor class initialized.")
        except ImportError as e:
            if self.logger:
                self.logger.error(f"Could not initialize MovieEditor class, error message: '{str(e)}'")
            else:
                print(f"[ERROR] Could not initialize MovieEditor class, error message: '{str(e)}'")
    ...
    def overlay(
        ...
        Args:
            ...
            subtitle_text (str): The text that is displayed over the raw_filepath.
                It can be either Chinese or English, but don't contain '\n' and '\t', and with fixed font and size. 
        ...
        if not os.path.isfile(image_filepath):
            warn_msg = f"overlay(), the input video file '{image_filepath}' doesn't exist."
            self.logger.warn(warn_msg)
            return
        else:
            image_extensions = ('.png', '.jpg', '.jpeg', '.bmp', '.gif')
        ...
        if len(subtitle_text.strip()) > 0:
            try:
                subtitle_clip = TextClip(
                    ...
                    color='white',  # Changed to string format
                    text_align="center",
                )

                subtitle_clip = subtitle_clip.with_position(subtitle_position, relative=True)
                subtitle_duration = subtitle_time_range[1] - subtitle_time_range[0]
                subtitle_clip = subtitle_clip.with_start(subtitle_time_range[0]).with_duration(subtitle_duration)
                clip_list.append(subtitle_clip)
            except Exception as e:
        ...
        if os.path.isfile(image_filepath):
            try:
                ...
                image_clip = image_clip.with_start(image_time_range[0]).with_duration(image_duration)
    ...
    def usage_sample(self):
        ...
        self.concatenate([
                output_video_filepaths[3],
                output_video_filepaths[5],
                output_video_filepaths[1],
                output_video_filepaths[6],
                output_video_filepaths[4]
            ], 
