...
class RockGenerator:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.texture_applier = TextureShader()
    ...
    def create_rock(self, scale=(1.0, 1.0, 1.0), skew=(0.0, 0.0, 0.0)):
        ...
        try:
            ...
            self.logger.info(f"create_rock(), created rock: {self.rock_object.name}")
        except RuntimeError as e:
            self.logger.warning(f"create_rock(), ailed to create rock. Is the addon enabled? Error: '{str(e)}'")
            self.rock_object = None
    ...
    def apply_texture(self, texture_directory):
        ...
        if not self.rock_object:
            self.logger.warning(f"apply_texture(), No rock object to texture. Please use create_rock() first.")
            return
        ...
        try:
            ...
            self.logger.info(f"apply_texture(), Applied textures from {texture_directory} to {self.rock_object.name}.")
            
        except (ValueError, FileNotFoundError) as e:
            self.logger.warning(f"apply_texture(), Failed to apply preliminary textures: '{str(e)}'.")
    ...
    def apply_secondary_texture(self, texture_directory):
        ...
        """
        if not self.rock_object:
            self.logger.warning(f"apply_secondary_texture(), No rock object to texture. Please use create_rock() first.")
            return
        ...
        try:
            ...
            info_msg = f"apply_secondary_texture(), Secondary textures '{texture_directory}' "
            info_msg += f"was applied to '{self.rock_object.name}'."
            self.logger.info(info_msg)

        except (ValueError, FileNotFoundError) as e:
            self.logger.warning(f"apply_secondary_texture(), Failed to apply secondary textures:'{str(e)}'")
    ...
    def scale_rock(self, scale=(1.0, 1.0, 1.0)):
        ...
        self.logger.info(f"scale_rock(), Scaling {self.rock_object.name} by {scale}. ")
        self.rock_object.scale = scale
    ...
    def rotate_rock(self, angle=(0, 0, 0)):
        ...
        self.logger.info(f"rotate_rock(), Rotating {self.rock_object.name} by {angle} radians.")
