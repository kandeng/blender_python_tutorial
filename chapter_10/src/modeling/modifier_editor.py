...
class ModifierEditor:
    """
    Create modifiers, and set their attributes.
    Also demonstrate the basic usage of various modifiers,
    subdivision (SUBSURF), smooth(SMOOTH), displace(DISPLACE) 
    """
    def __init__(
            self, 
            obj=None
        ...
        try:
            ...
            if self.obj and hasattr(self.obj, 'type') and self.obj.type == 'MESH':
                self.logger.info(f"Create a ModifierEditor for a mesh object named '{self.obj.name}'.")
            else:
                self.logger.info(f"Create a ModifierEditor, but need to set a mesh object to it.")
        except Exception as e:
            print("[ERROR] Could not initialize the ModifierEditor.")
    ...
    def add_modifier(
        ...
        for mod in self.obj.modifiers:
            if modifier_name.upper() == mod.name.upper():
                debug_msg = f"add_modifier(), A modifier named '{modifier_name}' already exists, " 
                debug_msg += f"return the existing modifier."
    ...
    def get_modifier_stack(self) -> list:
        modifier_stack = [m.name for m in self.obj.modifiers]

        debug_msg = f"get_modifier_stack(), object '{self.obj.name}'s modifier stack, \n"
        modifier_stack_str = json.dumps(modifier_stack, ensure_ascii=False, indent=2)
        debug_msg += modifier_stack_str
        self.logger.debug(debug_msg)

        return modifier_stack
    

    def move_modifier_in_stack(
            self, 
            modifier_name:str="",
            target_index: int=0
        ):
        # Set self.obj to be the active object.
    ...
    def usage_demo():
        ...
        #
        modifier_gen = ModifierEditor(obj=plane)
        ...
        # 4. Add smooth modifier (to enhance displacement effect)
        # 
        smooth_attrs = {
            "factor": 0.599
        }
        ...
        # 7. Move up or down the modifier in the modifier stack
        modifier_gen.get_modifier_stack()
        modifier_gen.move_modifier_in_stack(modifier_name="DisplaceMod", target_index=1)
        modifier_gen.get_modifier_stack()
        modifier_gen.move_modifier_in_stack(modifier_name="SubsurfMod", target_index=2)
        modifier_gen.get_modifier_stack()

        print("\n[INFO] Modifier creation and configuration completed. \n\n")


if __name__ == "__main__":
    ModifierEditor.usage_demo()