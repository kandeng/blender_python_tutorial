...
class ApplyTexture:
    ...
    def create_base_nodes(self):
        ...
        # Set up the texture generator with the object and material
        self.texture_generator.set_object(self.mesh)
        # Note: We're not calling create_material here as we've already created the material above
        self.texture_generator.material = self.material
        self.texture_generator.node_tree = self.material.node_tree

        # Set proper displacement method for Cycles
        self.material.cycles.displacement_method = 'BOTH'  # Use both bump and true displacement

        # Create the main Principled BSDF shader node
        principled_bsdf_node_name = "Principled_BSDF_Node"  # Use the expected name
        ...
        if tex_coord_node and mapping_node:
            self.texture_generator.create_link_via_sockets(
                tex_coord_node.outputs['UV'],  # UV output
                mapping_node.inputs['Vector']   # Vector input
            )

        # Create and link the material output node
        material_output_node_name = "Output_Node"  # Use the expected name
        material_output_node = self.texture_generator.create_node(
            'ShaderNodeOutputMaterial',
            material_output_node_name,
            location=(300, 0)
        )
        ...
        if self.principled_bsdf and material_output_node:
            self.texture_generator.create_link_via_sockets(
                self.principled_bsdf.outputs[0],  # BSDF
                material_output_node.inputs[0]   # Surface
            )
        else:
            print("[ERROR] Failed to create shader nodes.")

        print("[INFO] Base shader nodes created.")
    ...
    def create_displacement_modifier(self, disp_strength:float=0.1, midlevel:float=0.5):
        ...
        subsurf = self.mesh.modifiers.new(name="DisplacementSubdivision", type='SUBSURF')
